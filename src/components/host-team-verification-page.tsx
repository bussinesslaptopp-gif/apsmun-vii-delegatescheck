...
export function HostTeamVerificationPage({ department }: HostTeamVerificationPageProps) {
  ...
  useEffect(() => {
    const fetchAndParseData = async () => {
      ...
      const storageKey = `host-team-${department}`;
      ...
      if (cachedData) {
        ...
        toast({
          ...
          description: `${parsedData.length} members loaded for ${department}.`,
        });
        return;
      }
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error('Could not load host team data file. Make sure `host-team.xlsx` is in the `public` folder.');
        }
        ...
        if (!('ID' in firstRow && 'Name' in firstRow && 'Department' in firstRow)) {
          setLoadingError("Invalid Excel format. It must contain 'ID', 'Name', and 'Department' columns.");
          setDataLoaded(true);
          return;
        }
        ...
        const departmentMembers = allMembers.filter(member => member.Department === department);

        setMembers(departmentMembers);
        localStorage.setItem(storageKey, JSON.stringify(departmentMembers));
        setDataLoaded(true);
        toast({
          title: 'Success',
          description: `${departmentMembers.length} members loaded for ${department}.`,
        });
      } catch (e: any) {
