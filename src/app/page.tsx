    ...
    const fetchAndParseData = async () => {
        ...
        const parseAndSetData = (jsonData: any[], source: 'cache' | 'network') => {
            ...
            if (!('DelegateNo' in firstRow && 'Name' in firstRow)) {
                throw new Error("Invalid Excel format. It must contain 'DelegateNo' and 'Name' columns.");
            }
            ...
            if (source === 'network') {
                ...
                toast({
                    ...
                    description: `${formattedData.length} delegates loaded from the server.`,
                });
            } else {
                 toast({
                    title: "Loaded from Cache",
                    description: `${formattedData.length} delegates loaded. You are offline.`,
                });
            }
        };
        ...
        try {
            if (navigator.onLine) {
                const response = await fetch(`/delegates.xlsx?_=${new Date().getTime()}`);
                if (!response.ok) throw new Error('Could not load delegate data file.');
                ...
                throw new Error("Offline mode"); // Jump to cache loading
            }
            ...
            if (cachedData) {
                try {
                    ...
                    setLoadingError("Failed to load data. Cache is corrupted.");
                    setDataLoaded(true);
                }
            } else {
                setLoadingError(networkError.message === 'Offline mode' ? "You are offline and no data is cached." : "Failed to load data. Please check your connection.");
                setDataLoaded(true);
            }
        }
    };
