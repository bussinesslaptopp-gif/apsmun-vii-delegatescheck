...
type UniversalItem = Delegate | HostMember;
...
export default function VerifyZipPage() {
  ...
  useEffect(() => {
    ...
    const dcMembers = JSON.parse(localStorage.getItem('host-team-DC') || '[]');
    const ecMembers = JSON.parse(localStorage.getItem('host-team-EC') || '[]');

    const combinedList = [...delegates, ...dcMembers, ...ecMembers];
    ...
    if (combinedList.length > 0) {
      ...
      toast({
        variant: 'destructive',
        title: 'No data found',
        description: 'Please go back to the main page and load all data first.',
      });
      router.push('/');
    }
  }, [router, toast]);
  ...
  const findItem = (query: string): UniversalItem | undefined => {
      const upperQuery = query.trim().toUpperCase();
      return allItems.find(item => {
          if ('DelegateNo' in item) {
              return item.DelegateNo.toUpperCase() === upperQuery;
          }
          if ('ID' in item) {
              return item.ID.toUpperCase() === upperQuery;
          }
          return false;
      });
  }
  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    ...
    for (const imageFile of imageFiles) {
        ...
        try {
            ...
            if (item) {
                results.push({ fileName: imageFile.name, status: 'verified', decodedText, item, imageData });
            } else {
                localFailedCount++;
    ...
    <div>
      ...
      <header className="flex items-center mb-8 gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">Upload a ZIP file to automatically scan and verify every QR code.</p>
        </div>
      </header>
          ...
          <div {...getRootProps()} className={`flex flex-col items-center justify-center p-10 border-2 border-dashed rounded-lg cursor-pointer transition-colors ${isDragActive ? 'border-primary bg-primary/10' : 'border-border hover:border-primary/50'}`}>
            ...
            {isVerifying ? (
                ...
                <>
                    ...
                    <p className="text-lg font-semibold">Drop your QR code ZIP here, or click to select</p>
      ...
      {verificationResults.length > 0 && !isVerifying && (
         <div className="space-y-8">
            ...
            {failedResults.length > 0 && (
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>These QR codes could not be scanned or did not match any person.</CardDescription>
                    </CardHeader>
                    <CardContent>
            ...
            {verifiedResults.length > 0 && (
                 <Card>
                    <CardHeader>
                        <CardTitle className="text-green-500 flex items-center gap-2"><CheckCircle/> Verified People ({verifiedResults.length})</CardTitle>
                       ...
                       {verifiedResults.map(result => (
                         <div key={result.fileName} className="flex items-center justify-between p-2 bg-muted/50 rounded-md">
                           <div className="flex items-center gap-3">
                                <Badge variant="secondary">{'DelegateNo' in result.item! ? result.item.DelegateNo : result.item!.ID}</Badge>
                                <div>
